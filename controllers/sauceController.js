...
const saucePutOne = async (req, res) => {
	...
	try {
		...
		const sauceUpdate = await sauceModel
			.findOneAndUpdate({ _id: req.params.id, userId: req.userId }, { $set: sauce })
			.exec()
		if (sauceUpdate == null) {
...
const sauceLike = async (req, res) => {
	try {
		const data = await sauceModel.findOne({ _id: req.params.id })
		...
		await sauceModel.updateOne({ _id: req.params.id }, productLikes)
